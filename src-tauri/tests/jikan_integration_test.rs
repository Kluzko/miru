...
async fn test_get_anime_by_id_success() {
    ...
    match result {
        Ok(Ok(Some(anime))) => {
            ...
            // UUID field exists, no need to compare with 0
...
async fn test_get_anime_full() {
    ...
    let result = timeout(
        TEST_TIMEOUT,
        ProviderAdapter::get_anime_full(&adapter, POPULAR_ANIME_ID),
    )
    .await;
...
async fn test_get_season_now() {
    ...
    let result = timeout(TEST_TIMEOUT, ProviderAdapter::get_season_now(&adapter, 3)).await;
...
async fn test_get_season_upcoming() {
    ...
    let result = timeout(
        TEST_TIMEOUT,
        ProviderAdapter::get_season_upcoming(&adapter, 3),
    )
    .await;
    ...
    match result {
        Ok(Ok(characters)) => {
            ...
            for character in characters.iter().take(3) {
                let char_info = &character.character;
    ...
    match result {
        Ok(Ok(staff)) => {
            ...
            for staff_member in staff.iter().take(3) {
                let person = &staff_member.person;
                println!("  - {}", person.name);
            }
        }
        Ok(Err(e)) => {
