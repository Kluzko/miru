...
use crate::modules::anime::domain::{
    entities::{anime_detailed::AnimeDetailed, genre::Genre},
    repositories::anime_repository::AnimeRepository,
    value_objects::quality_metrics::QualityMetrics,
};
...
use super::super::mapper::{
    entity_to_changeset, entity_to_new_model, model_to_entity, model_to_entity_with_external_ids,
};
...
impl AnimeRepositoryImpl {
    ...
    /// Load anime batch with all related data (genres, studios, quality metrics, external IDs)
        ...
        let results = task::spawn_blocking(move || -> AppResult<Vec<AnimeDetailed>> {
            ...
            let out = anime_models
                ...
                .map(|(m, mvec)| {
                    ...
                    model_to_entity_with_external_ids(
                        m,
                        genres,
                        studios,
    ...
    async fn upsert_anime(&self, anime: &AnimeDetailed) -> AppResult<Anime> {
        let db = Arc::clone(&self.db);
        let new_anime = entity_to_new_model(anime);
        let changes = entity_to_changeset(anime);
    ...
    fn upsert_external_ids_blocking(
        ...
        anime_id: Uuid,
        provider_metadata: &crate::shared::domain::value_objects::ProviderMetadata,
    ) -> AppResult<()> {
        use crate::schema::anime_external_ids;
        use diesel::prelude::*;
...
// -------------------------------------------------------------------------
// Public API implementation for AnimeRepository trait
// -------------------------------------------------------------------------
#[async_trait]
impl AnimeRepository for AnimeRepositoryImpl {
    ...
    async fn save_batch(&self, anime_list: &[AnimeDetailed]) -> AppResult<Vec<AnimeDetailed>> {
        ...
        let saved_models = task::spawn_blocking(move || -> AppResult<Vec<Anime>> {
            ...
            conn.transaction::<Vec<Anime>, AppError, _>(|conn| {
                ...
                let new_anime_records: Vec<NewAnime> =
                    to_upsert.iter().map(|a| entity_to_new_model(a)).collect();
    ...
    async fn save_relations(
        &self,
        _anime_id: &Uuid,
        _relations: &[(Uuid, String)],
    ) -> AppResult<()> {
        // This is a placeholder - actual implementation is in anime_relations_repository_impl.rs
        log_warn!("save_relations called on AnimeRepositoryImpl - this should be delegated to AnimeRelationsRepositoryImpl");
        Ok(())
    }
    ...
    async fn get_anime_with_relations(
        &self,
        _anime_id: &Uuid,
    ) -> AppResult<Vec<crate::modules::anime::domain::repositories::anime_repository::AnimeWithRelationMetadata>>{
        // This is a placeholder - actual implementation is in anime_relations_repository_impl.rs
        log_warn!("get_anime_with_relations called on AnimeRepositoryImpl - this should be delegated to AnimeRelationsRepositoryImpl");
        Ok(Vec::new())
    }
}
